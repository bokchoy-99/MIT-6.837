...
Vec3f RayTracer::traceRay(Ray &ray, float tmin, int bounces, float weight, float indexOfRefraction, Hit &hit) const
{
    ...
    if (castRay(ray, hit, tmin))
    {
        ...
        // 物体在环境光中的颜色
        color = ambient * material->getDiffuseColor(intersection);
        // 处理各个光源的贡献
        for (int i = 0; i < numLights; i++)
        {
            ...
            if (cmdlParser->cast_shadow)
            {
                ...
                if (castRay(shadowRay, shadowHit, eps * 100.f) && shadowHit.getT() < distToLight)
                {
        ...
        // 处理反射光，进行递归
        auto reflectColor = material->getReflectiveColor(intersection);
        ...
        // // 处理折射光，进行递归
        auto transparentColor = material->getTransparentColor(intersection);
        // 如果是 entering transmissive material，即 ray * norm < 0, 取材料的折射率
        // 如果是 leaving transmissive material，即 ray * norm > 0, 折射率取 1.f, 因为当前在材料内
        auto materialIndexOfRefraction = isBackSide ? 1.f : material->getIndexOfRefraction(intersection);
...
bool RayTracer::castRay(Ray &ray, Hit &hit, float tmin) const
{
    if (cmdlParser->grid_nx > 0 && cmdlParser->grid_ny > 0 && cmdlParser->grid_nz > 0)
