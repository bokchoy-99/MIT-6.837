...
bool Grid::intersect(const Ray &r, Hit &h, float tmin)
{
    ...
    // 标记已经处理过的对象
    unordered_set<Object3D *> visitedObjectsSet;
            ...
            if (!cell.empty())
            {
                if (visualizeGrid)
                {
                    ...
                    if (!firstHit)
                    {
                        ...
                        auto nObjects = static_cast<int>(cell.size());
                        nHit.Negate();
                        h.set(mi.tmin, getCellMaterial(std::min(nObjects, SchemaColorNumber) - 1), nHit, r, ObjectType::GridObject);
                        firstHit = true;
                    }
                }
                else
                {
                    // 与 cell 中的 object 相交
                    auto nextT = min(min(mi.t_next[0], mi.t_next[1]), mi.t_next[2]);
                    Hit closestHit;
    ...
    bool intersectionWithInfiniteObjects = false;
    if (!visualizeGrid)
    {
        for (auto object : infiniteObjects)
        {
            ...
            if (object->intersect(r, infiniteObjectHit, tmin))
            {
                ...
                {
                    intersectionWithInfiniteObjects = true;
                    h = infiniteObjectHit;
                }
            }
        }
    }
    return mi.hit || intersectionWithInfiniteObjects;
}
